...
1. **Nenhuma dependência de runtime além de `react` e `react-dom`.** Antes de instalar qualquer coisa, verifique se dá para escrever. GSAP, Three.js, Framer Motion e afins foram avaliados e dispensados: o movimento vive em `src/lib/motion.ts` e o objeto 3D em `src/lib/nucleo.ts`.
...
- `subscribe()` em `src/lib/motion.ts` é o único `requestAnimationFrame` da página. Nunca crie outro loop.
